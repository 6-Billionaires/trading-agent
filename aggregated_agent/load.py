...
import argparse
import config
import aggregated_agent.ssa_metrics as mt
...
def load_model(model_type):
    ...
    if model_type == 'boa':
        ...
        model = build_boa_network(max_len, neurons=15, activation='leaky_relu')
    ...
    if model_type == 'ssa':
        ...
        model.load_weights('aggregated_agent/networks/ssa.h5f')  # << 수정완료 n125
        return model

    if model_type == 'soa':
        max_len = util.get_maxlen_of_binary_array(120)
        model = build_soa_network(max_len, neurons=80, activation='leaky_relu')
        model.compile(optimizer='adam', loss='mse', metrics=['mae', 'mape'])
        model.load_weights('aggregated_agent/networks/soa.h5f')  # << 수정완료 n80
        return model
...
def build_ssa_network(optimizer='adam',init_mode='uniform',
