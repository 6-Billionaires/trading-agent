...
class Agents:
    ...
    def append_sample(self, state, action, reward, next_state, done):
        if action == 0:  # action 이 0 인 경우 additional reward 가 없으므로 그냥 memory 에 sample 추가
            ...
            self.sample_buffer.append([state, action, reward, next_state, done])
        self._sequence_manage(action)

    def _append_buffer_sample(self):
        self.sample_buffer[0][2] += self.sample_buffer[1][2] * 0.1 + self.sample_buffer[3][2] * 0.1
        self.sample_buffer[1][2] += self.sample_buffer[3][2] * 0.1
        self.sample_buffer[2][2] += self.sample_buffer[3][2] * 0.1
...
class MyTGym(tgym.TradingGymEnv):  # MyTGym 수정해야 함 -> agent 별 reward 를 줘야 함 (4개 반환해서 agents 가 수정하거나 agent 입력해서 reward 주거나)
    def _rewards(self, observation, action, done, info):
        return [0.1, 0, -0.4, 0.3][random.randint(0, 3)]
...
if __name__ == '__main__':
    ...
    for ep in range(EPISODES):
        ...
        while not done:
            ...
            next_state, reward, done, info = env.step(action)  # reward 산정을 위해 어떤 agent 를 사용했는지 gym 에 알려줘야 할듯
